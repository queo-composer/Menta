...
class Selenium1Facade extends AbstractComponentTest
{
    ...
    public function waitForElementPresent($locator, $timeout = null, $message = null)
    {
        if (is_null($message)) {
            $message = sprintf(
                "Waiting for element '%s' timed out",
                $this->getHelperCommon()->element2String($locator)
            );
            if (!is_null($timeout)) {
    ...
    public function waitForElementNotPresent($locator, $timeout = null, $message = null)
    {
        if (is_null($message)) {
            $message = sprintf(
                "Waiting for element '%s' disappearing timed out",
                $this->getHelperCommon()->element2String($locator)
            );
            if (!is_null($timeout)) {
                $message .= " after $timeout seconds";
    ...
    { #
        if (is_null($message)) {
            $message = sprintf(
                "Waiting for element '%s' visible timed out",
                $this->getHelperCommon()->element2String($locator)
            );
            if (!is_null($timeout)) {
                $message .= " after $timeout seconds";
    ...
    public function waitForNotVisible($locator, $timeout = null, $message = null)
    {
        if (is_null($message)) {
            $message = sprintf(
                "Waiting for element '%s' not visible timed out",
                $this->getHelperCommon()->element2String($locator)
            );
            if (!is_null($timeout)) {
                $message .= " after $timeout seconds";
    ...
    public function clickAndWaitAjax($clickElement, $waitForElementAfterClick, $timeout = null)
    {
        $this->getTest()->assertTrue(
            $this->isElementPresent($clickElement),
            'element: ' . $clickElement . ' is not present'
        );
